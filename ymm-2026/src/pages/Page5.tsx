...
export default function Page5() {
  ...
  useEffect(() => {
    fetch('/data/page5_data.csv')
      .then((res) => {
        if (!res.ok) throw new Error(`Failed to load page5 data: ${res.status}`)
        return res.text()
      })
  ...
  return (
    <div className="page-shell">
      <BackGroundImage src="/resources/background_5.png" align="top-center" />
      <div ref={gridRef} className="table-grid table-grid--page5" style={gridStyle}>
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <TableCellItem variant="crown" name={item.name} message={item.message} />
            </div>
          )
        })}
      </div>
