...
export default function Page2() {
  ...
  useEffect(() => {
    fetch('/data/page2_data.csv')
      .then((res) => {
        if (!res.ok) throw new Error(`Failed to load page2 data: ${res.status}`)
        return res.text()
      })
  ...
  return (
    <div className="page-shell">
      <BackGroundImage src="/resources/background_2.png" />
      <div ref={gridRef} className="table-grid table-grid--page2" style={gridStyle}>
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <TableCellItem variant="comment" name={item.name} message={item.message} />
            </div>
          )
        })}
      </div>
