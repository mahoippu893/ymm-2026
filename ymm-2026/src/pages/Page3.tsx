...
export default function Page3() {
  ...
  useEffect(() => {
    fetch('/data/page3_data.csv')
      .then((res) => {
        if (!res.ok) throw new Error(`Failed to load page3 data: ${res.status}`)
        return res.text()
      })
  ...
  return (
    <div className="page-shell">
      <BackGroundImage src="/resources/background_3.png" />
      <div ref={gridRef} className="table-grid table-grid--page3" style={gridStyle}>
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <TableCellItem variant="spade" name={item.name} message={item.message} />
            </div>
          )
        })}
      </div>
