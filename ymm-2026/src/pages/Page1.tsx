...
export default function Page1() {
  ...
  useEffect(() => {
    fetch('/data/page1_data.csv')
      .then((res) => {
        if (!res.ok) throw new Error(`Failed to load page1 data: ${res.status}`)
        return res.text()
      })
  ...
  return (
    <div className="page-shell">
      <BackGroundImage src="/resources/background_1.png" align="top-center" />
      <div ref={gridRef} className="table-grid table-grid--page1" style={gridStyle}>
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <TableCellItem variant="heart" name={item.name} message={item.message} />
            </div>
          )
        })}
      </div>
